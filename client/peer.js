...
export default function Peer(opts) {
  ...
  function ensurePeerConnection({ caller }) {
    if (pc) {
    ...
    return turnServerPromise.then(turnServers => {
      ...
      if (caller) {
        const dataChannel = pc.createDataChannel('ledoot');

        dataChannel.onerror = evt => {
          console.log('data channel on error', evt);
        };

        dataChannel.onmessage = evt => {
          console.log('data channel on message', evt);
        };

        dataChannel.onopen = evt => {
          console.log('data channel on open', evt);
        };

        dataChannel.onclose = evt => {
          console.log('data channel on close', evt);
        };
      } else {
      ...
      pc.onicecandidate = (evt) => {
        ...
        console.log('transmitting icecandidate', evt);
        socket.emit('signal', {
          type: 'icecandidate',
          to: peerId,
  ...
  function shareMedia(constraints = { audio: true, video: true }) {
    ensurePeerConnection({ caller: true }).then(() => {
  ...
  function handleOffer(signal) {
    ...
    ensurePeerConnection({ caller: false }).then(() => {
